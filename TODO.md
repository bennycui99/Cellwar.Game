...
### X - 改动模型。
#### 1改动 coding gene.delta：delta 可以编辑，具有加减乘除运算，并且可以获取block.chemical.count、strain.population、privateScource.chemical.count。
...
添加coding gene效果：改变private resource的物质（和改变ProductionChemicalInfo是并列的，但ProductionChemicalInfo是改变map block chemical，而这个可以改变private chemical）。

#### 3改动regulatory gene 的 Condition：类似逻辑运算符，由and，or，正负，物质数量要求 四个要素组成，并且可以嵌套。
